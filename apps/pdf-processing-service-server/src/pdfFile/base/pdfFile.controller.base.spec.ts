...
const service = {
  ...
  pdfFiles: () => FIND_MANY_RESULT,
  pdfFile: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("PdfFile", () => {
  ...
  test("GET /pdfFiles/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/pdfFiles"}/${nonExistingId}`)
  ...
  test("GET /pdfFiles/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/pdfFiles"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /pdfFiles existing resource", async () => {
